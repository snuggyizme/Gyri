...
def _manageArgs(args: tuple | list, expect: list):
    ...
    if isinstance(input, tuple):
        ...
        args, newIndex = utils._getArguments(string, index, expect, TAPE)
        index = newIndex
...
def pset(inp: tuple):
    ...
    amount = args[0]
    if amount == None:
        utils._throw("No argument found for command p:set", index if isinstance(index, int) else "Unknown index - outside of script")
...
def pinc(inp: tuple):
    ...
    amount = args[0]
    if amount == None:
        utils._throw("No argument found for command p:inc", index if isinstance(index, int) else "Unknown index - outside of script")
...
def pdec(inp: tuple):
    ...
    amount = args[0]
    if amount == None:
        utils._throw("No argument found for command p:dec", index if isinstance(index, int) else "Unknown index - outside of script")
...
def pjmp(inp: tuple):
    ...
    dst = args[0]
    if dst == None:
        utils._throw("No argument found for command p:jmp", index if isinstance(index, int) else "Unknown index - outside of script")
...
def iprt(inp: tuple):
    ...
    item = args[0]
    if item == None:
        utils._throw("No argument found for command i:prt", index if isinstance(index, int) else "Unknown index - outside of script")
...
def iprn(inp: tuple):
    ...
    item = args[0]
    if item == None:
        utils._throw("No argument found for command i:prn", index if isinstance(index, int) else "Unknown index - outside of script")
    elif isinstance(item, list): #  Range case
        for i in item:
            print(item[i])
    elif isinstance(item, tuple): # Coord case
    ...
    while i < len(code):
        ...
        match c.lower():
            ...
            case "w":
                if code[i+1] == ":":
                    break
                count, n = utils._parseNumber(code, i)
            ...
            # ======================================================================
            case "@":
                combinedCoordinate, newIndex, _ = utils._parseCoordinate(code, i, TAPE)
            ...
            # ======================================================================
            case "#":
                command, newIndex = utils._parseName(code, i, "<")
                i = newIndex

                match command:
                    case "include":
                        args, newIndex = utils._getArguments(code, i, [["str"]], TAPE, ("<", ">"))
                        i = newIndex
                        
                        if not Path(f"{args[0]}.json").is_file():
                            utils._throw(f"Could not find set at path {args[0]}", i)
        ...
        if c in (string.ascii_lowercase + "0123456789") and code[i+1] == ":":
            ...
            if c == "p":
                match instruction:
                    ...
                    case _:
                        utils._throw(f"Unknown instruction '{instruction}' in set '{c}'", i)
                ...
                utils._throw("Unknown key: make sure you installed and imported the set correctly, and that you are using the correct set key", i) 
